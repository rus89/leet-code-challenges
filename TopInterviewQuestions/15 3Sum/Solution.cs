namespace TopInterviewQuestions._153Sum;
...
public class Solution
{
    public IList<IList<int>> ThreeSum(int[] nums)
    {
        ...
        /*The start goes from 0 to length-2 because look here
         -4 -2 -3 -1 0 0 0 2 3 10 21
                             s  l  r      */
        while (start < nums.Length - 2)
        {
            ...
            while (left < right)
            {
                /*The array is sorted, so if we move to the left the right index, the sum will decrease */
                if (nums[left] + nums[right] > target)
                {
                    --right;
                }
                /*Here is the opposite, it the sum of nums[l] and nums[r] is less that what we are looking for,
